...
const SideBar = ({ user, onLogout, onLogin }) => {
  ...
  return (
    <div className="fixed top-0 flex md:flex:row flex-col left-0 md:left-auto  h-16 w-screen md:w-1/4 md:h-screen bg-gray-800 text-white  md:items-start p-2">
      <div className="py-2 md:py-4 font-bold text-lg md:text-3xl text-center ">
        WATCHLIST
      </div>
      ...
      <div className="flex flex-row border  border-gray-800 items-center w-full bg-gray-300 py-2 px-2 md:py-4 md:px-4 text-black rounded-md md:my-2">
        <img
          className=" h-6 md:h-8 md:px-2"
          alt="user"
          src="https://www.iconpacks.net/icons/2/free-user-icon-3296-thumb.png"
        />
        <div className=" w-0 h-0 md:w-auto md:h-auto flex-grow md:flex-grow-0 md:px-2 hidden md:block">
          User
        </div>
        <div className=" ml-1 flex-1 text-right">
          {user ? (
            <button
              className="md:bg-red-600 text:black md:text-white px-1 py-1 md:px-2 md:py-2 rounded-md"
              onClick={onLogout}
            >
              Logout
            </button>
            ...
            <button
              className="md:bg-green-600 text-black md:text-white px-1 py-1 md:px-2 md:py-2 rounded-md"
              onClick={onLogin}
            >
              Login
            </button>
        ...
        <div className=" m-1 md:ml-2  w-1/5 md:w-auto text-right">
          <button
            className="md:bg-red-500 text-black md:text-white md:px-2 md:py-1 rounded-md"
      ...
      <div
        className={`${
          showWatchlist ? "block" : "hidden"
        } md:block w-full mt-1 md:mt-4`}
      >
        <Watchlist />
      </div>
    </div>
